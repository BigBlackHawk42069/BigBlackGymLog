    ...
    function renderExpBar() {
        const wrap = document.getElementById('bbgl-exp-bar-wrapper');
        if (!wrap) return;
        let totalExp = runtime.careerLevelExp || 0;
        const s = getActiveHistory();
        if (s && s.today) {
            const e = s.today.cost || (s.today.eSpent ? s.today.eSpent.total : 0);
            const hasTrainLog = s.today.series && s.today.series.some(x => x.type === 'gym');
            totalExp += computeDailyLevelExp(e, hasTrainLog);
        }
        const { level, expInLevel, expToNext } = calculateLevelProgress(totalExp);
        let pct = expToNext > 0 ? (expInLevel / expToNext) * 100 : 100;
        pct = Math.max(0, Math.min(100, pct));
        const fillLayer = wrap.querySelector('.bbgl-fill');
        if (fillLayer) fillLayer.style.setProperty('--exp-pct', `${pct}%`);
        const textLayers = wrap.querySelectorAll('.bbgl-text .bbgl-exp-level');
        textLayers.forEach(el => el.textContent = `Lv. ${level}`);
    }
